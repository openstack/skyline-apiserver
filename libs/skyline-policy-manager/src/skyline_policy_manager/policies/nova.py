...
list_rules = (
    ...
    base.APIRule(
        ...
        "APIs are proxy calls to the Ironic service and are "
        "deprecated.\n#",
        scope_types=["system"],
    ...
    base.APIRule(
        ...
        "for a given console authentication token",
        scope_types=["system"],
    ...
    base.APIRule(
        ...
        "will control the visibility for a set of servers "
        "attributes:\n#\n#- ``OS-EXT-SRV-ATTR:host``\n#- "
        "``OS-EXT-SRV-ATTR:instance_name``\n#- "
        "``OS-EXT-SRV-ATTR:reservation_id`` (since microversion "
        "2.3)\n#- ``OS-EXT-SRV-ATTR:launch_index`` (since "
        "microversion 2.3)\n#- ``OS-EXT-SRV-ATTR:hostname`` ("
        "since microversion 2.3)\n#- "
        "``OS-EXT-SRV-ATTR:kernel_id`` (since microversion "
        "2.3)\n#- ``OS-EXT-SRV-ATTR:ramdisk_id`` (since "
        "microversion 2.3)\n#- "
        "``OS-EXT-SRV-ATTR:root_device_name`` (since "
        "microversion 2.3)\n#- ``OS-EXT-SRV-ATTR:user_data`` ("
        "since microversion 2.3)\n#\n#Microvision 2.75 added the "
        "above attributes in the ``PUT /servers/{"
        "server_id}``\n#and ``POST /servers/{server_id}/action ("
        "rebuild)`` API responses which are\n#also controlled by "
        "this policy rule, like the ``GET /servers*`` APIs.\n#",
        scope_types=["system", "project"],
        operations=[
    ...
    base.APIRule(
        ...
        "the full list of tenants that have access\n#to a flavor "
        "via an os-flavor-access API.\n#",
        scope_types=["system"],
    ...
    base.APIRule(
        ...
        "microversion 2.47, the flavor used for a server is also "
        "returned in the response when showing server details, "
        "updating a server or rebuilding a server. Starting with "
        "microversion 2.61, extra specs may be returned in "
    ...
    base.APIRule(
        ...
        scope_types=["system", "project"],
        operations=[
            {"method": "POST", "path": "/servers/{server_id}/action (removeFloatingIp)"},
        ],
    ),
    base.APIRule(
    ...
    base.APIRule(
        ...
        check_str=("rule:system_reader_api"),
        description="List physical hosts.\n#\n#This API is deprecated in "
        "favor of os-hypervisors and os-services.",
        scope_types=["system"],
    ...
    base.APIRule(
        ...
        "deprecated in favor of os-hypervisors and os-services.",
        scope_types=["system"],
    ...
    base.APIRule(
        ...
        "in favor of os-hypervisors and os-services.",
        scope_types=["system"],
    ...
    base.APIRule(
        ...
        check_str=("rule:system_reader_api"),
        description="List all servers on hypervisors that can match the "
        "provided hypervisor_hostname pattern.",
        scope_types=["system"],
        operations=[
    ...
    base.APIRule(
        ...
        check_str=("rule:system_reader_api"),
        description='Add "details" key in action events for a '
        "server.\n#\n#This check is performed only after the "
        "check\n#os_compute_api:os-instance-actions:show passes. "
        "Beginning with Microversion\n#2.84, new field 'details' "
        "is exposed via API which can have more details "
        "about\n#event failure. That field is controlled by this "
        "policy which is system reader\n#by default. Making the "
        "'details' field visible to the non-admin user helps "
        "to\n#understand the nature of the problem (i.e. if the "
        "action can be retried),\n#but in the other hand it "
        "might leak information about the deployment\n#(e.g. the "
    ...
    base.APIRule(
        ...
        check_str=("rule:system_reader_api"),
        description="Add events details in action details for a "
        "server.\n#This check is performed only after the "
        "check\n#os_compute_api:os-instance-actions:show passes. "
        "Beginning with Microversion\n#2.51, events details are "
        "always included; traceback information is "
        "provided\n#per event if policy enforcement passes. "
        "Beginning with Microversion 2.62,\n#each event includes "
        "a hashed host identifier and, if policy "
    ...
    base.APIRule(
        ...
        check_str=("rule:system_reader_api"),
        description="List all usage audits occurred before a specified time "
        "for all servers on all compute hosts where usage "
        "auditing is configured",
        scope_types=["system"],
        operations=[
            {"method": "GET", "path": "/os-instance_usage_audit_log/{before_timestamp}"},
        ],
    ),
    base.APIRule(
    ...
    base.APIRule(
        ...
        check_str=("rule:system_reader_api"),
        description="Show rate and absolute limits of other "
        "project.\n#\n#This policy only checks if the user has "
        "access to the requested\n#project limits. And this "
        "check is performed only after the "
    ...
    base.APIRule(
        ...
        check_str=("rule:system_admin_api"),
        description="Unlock a server, regardless who locked the "
        "server.\n#\n#This check is performed only after the "
    ...
    base.APIRule(
        ...
        "proxy calls to the Network service. This "
        "is\n#deprecated.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        "is proxy calls to the Network service. This "
        "is\n#deprecated.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        "calls to the Network service. This is deprecated.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        check_str=("rule:system_or_project_reader"),
        description="Show network details.\n#\n#This API is proxy calls to "
        "the Network service. This is deprecated.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        check_str=("rule:system_admin_or_owner"),
        description="Generate a URL to access remove server "
        "console.\n#\n#This policy is for ``POST "
        "/remote-consoles`` API and below Server actions "
        "APIs\n#are deprecated:\n#\n#- ``os-getRDPConsole``\n#- "
        "``os-getSerialConsole``\n#- ``os-getSPICEConsole``\n#- "
        "``os-getVNCConsole``.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        scope_types=["system", "project"],
        operations=[
            {"method": "POST", "path": "/servers/{server_id}/action (addSecurityGroup)"},
        ],
    ),
    base.APIRule(
    ...
    base.APIRule(
        ...
        check_str=("rule:system_reader_api"),
        description="Show the NUMA topology data for a server with host NUMA "
        "ID and CPU pinning information",
        scope_types=["system"],
        operations=[{"method": "GET", "path": "/servers/{server_id}/topology"}],
    ),
    base.APIRule(
    ...
    base.APIRule(
        ...
        check_str=("rule:system_admin_api"),
        description="\n#Show a server with additional host status "
        "information.\n#\n#This means host_status will be shown "
        "irrespective of status value. If showing\n#only "
        "host_status UNKNOWN is desired, "
        "use the\n#``os_compute_api:servers:show:host_status"
        ":unknown-only`` policy rule.\n#\n#Microvision 2.75 "
        "added the ``host_status`` attribute in the\n#``PUT "
        "/servers/{server_id}`` and ``POST /servers/{"
        "server_id}/action (rebuild)``\n#API responses which are "
        "also controlled by this policy rule, like the\n#``GET "
        "/servers*`` APIs.\n#",
        scope_types=["system", "project"],
        operations=[
            {"method": "GET", "path": "/servers/{server_id}"},
    ...
    base.APIRule(
        ...
        check_str=("rule:system_admin_api"),
        description="\n#Show a server with additional host status "
        "information, only if host status "
        "is\n#UNKNOWN.\n#\n#This policy rule will only be "
        "enforced when "
        "the\n#``os_compute_api:servers:show:host_status`` "
        "policy rule does not pass for the\n#request. An example "
        "policy configuration could be where "
        "the\n#``os_compute_api:servers:show:host_status`` rule "
        "is set to allow admin-only and\n#the "
    ...
    base.APIRule(
        ...
        description="\n#Create a server on the specified host and/or "
        "node.\n#\n#In this case, the server is forced to launch "
        "on the specified\n#host and/or node by bypassing the "
        "scheduler filters unlike "
    ...
    base.APIRule(
        ...
        check_str=("rule:project_admin_api"),
        description="\n#Create a server on the requested compute service "
        "host and/or\n#hypervisor_hostname.\n#\n#In this case, "
        "the requested host and/or hypervisor_hostname "
        "is\n#validated by the scheduler filters unlike "
    ...
    base.APIRule(
        ...
        check_str=("rule:project_admin_api"),
        description="\n#This rule controls the compute API validation "
        "behavior of creating a server\n#with a flavor that has "
        "0 disk, indicating the server should be "
        "volume-backed.\n#\n#For a flavor with disk=0, the root "
        "disk will be set to exactly the size of the\n#image "
        "used to deploy the instance. However, in this case the "
        "filter_scheduler\n#cannot select the compute host based "
        "on the virtual image size. Therefore, 0\n#should only "
        "be used for volume booted instances or for testing "
        "purposes.\n#\n#WARNING: It is a potential security "
        "exposure to enable this policy rule\n#if users can "
        "upload their own images since repeated attempts "
        "to\n#create a disk=0 flavor instance with a large image "
        "can exhaust\n#the local disk of the compute (or shared "
        "storage cluster). See "
        "bug\n#https://bugs.launchpad.net/nova/+bug/1739646 for "
        "details.\n#",
        scope_types=["system", "project"],
        operations=[{"method": "POST", "path": "/servers"}],
    ),
    ...
    base.APIRule(
        ...
        "disabled for all users and recommended to be tested in "
        "a deployment for admin users before opening it up to "
        "non-admin users. Resizing within a cell is the default "
        "preferred behavior even if this is enabled. ",
        scope_types=["system", "project"],
        operations=[{"method": "POST", "path": "/servers/{server_id}/action (resize)"}],
    ),
    base.APIRule(
    ...
    base.APIRule(
        ...
        scope_types=["system", "project"],
        operations=[
            {"method": "GET", "path": "/servers/{server_id}/migrations/{migration_id}"},
        ],
    ),
    base.APIRule(
    ...
    base.APIRule(
        ...
        check_str=("rule:system_or_project_reader"),
        description="List volumes.\n#\n#This API is a proxy call to "
        "the Volume service. It is deprecated.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        check_str=("rule:system_admin_or_owner"),
        description="Create volume.\n#\n#This API is a proxy call to the "
        "Volume service. It is deprecated.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        "to the Volume service. It is deprecated.",
        scope_types=["system", "project"],
    ...
    base.APIRule(
        ...
        check_str=("rule:system_admin_or_owner"),
        description="Update a volume attachment.\n#New 'update' policy about "
        "'swap + update' request (which is possible\n#only "
        ">2.85) only <swap policy> is checked. We expect <swap "
        "policy> to be\n#always superset of this policy "
        "permission.\n#",
        scope_types=["system", "project"],
        operations=[
