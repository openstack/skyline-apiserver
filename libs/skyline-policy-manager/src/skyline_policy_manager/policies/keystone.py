...
list_rules = (
    ...
    base.APIRule(
        ...
        operations=[
            {
                "method": "GET",
                "path": "/v3/users/{user_id}/OS-OAUTH1/"
                "access_tokens/{access_token_id}/roles/{role_id}",
            },
        ],
    ),
    base.APIRule(
    ...
    base.APIRule(
        ...
        check_str=("role:reader and system_scope:all"),
        description="Get the entire domain configuration "
        "for a domain, an option group within a domain, "
        "or a specific configuration option within a group "
        "for a domain.",
        scope_types=["system"],
        operations=[
    ...
    base.APIRule(
        ...
        check_str=(""),
        description="Get security compliance domain configuration "
        "for either a domain or a specific option in a domain.",
        scope_types=["system", "domain", "project"],
        operations=[
    ...
    base.APIRule(
        ...
        check_str=(
            ...
            "or (role:reader and domain_id:%(target.group.domain_id)s "
            "and domain_id:%(target.domain.id)s)) "
            "and (domain_id:%(target.role.domain_id)s "
            "or None:%(target.role.domain_id)s)"
        ),
        description="Check a role grant between a target and an actor. "
        "A target can be either a domain or a project. "
        "An actor can be either a user or a group. "
        "These terms also apply to the OS-INHERIT APIs, "
        "where grants on the target are inherited to all projects "
        "in the subtree, if applicable.",
        ...
        operations=[
            ...
            {
                "method": "HEAD",
                "path": "/v3/OS-INHERIT/projects/{project_id}/users/"
            ...
            {
                "method": "HEAD",
                "path": "/v3/OS-INHERIT/projects/{project_id}/groups/"
            ...
            {
                "method": "HEAD",
                "path": "/v3/OS-INHERIT/domains/{domain_id}/groups/"
                "{group_id}/roles/{role_id}/inherited_to_projects",
            },
            {
                "method": "GET",
    ...
    base.APIRule(
        ...
        check_str=(
            "(role:reader and system_scope:all) or (role:reader "
            ...
            "domain_id:%(target.group.domain_id)s and domain_id:%("
            "target.project.domain_id)s) or (role:reader and domain_id:%("
            "target.group.domain_id)s and domain_id:%(target.domain.id)s) "
        ),
        description="List roles granted to an actor on a target. "
        "A target can be either a domain or a project. "
        "An actor can be either a user or a group. "
        "For the OS-INHERIT APIs, it is possible to "
        "list inherited role grants for actors on domains, "
        "where grants are inherited to all projects in the "
        "specified domain.",
        scope_types=["system", "domain"],
        operations=[
    ...
    base.APIRule(
        ...
        check_str=(
            ...
            "domain_id:%(target.user.domain_id)s and domain_id:%("
            "target.project.domain_id)s) or (role:admin and domain_id:%("
            "target.user.domain_id)s and domain_id:%(target.domain.id)s) or "
            "(role:admin and domain_id:%(target.group.domain_id)s and "
            "domain_id:%(target.project.domain_id)s) or (role:admin and "
            "domain_id:%(target.group.domain_id)s and domain_id:%("
            "target.domain.id)s)) and (domain_id:%(target.role.domain_id)s "
            "or None:%(target.role.domain_id)s) "
        ),
        ...
        "to the OS-INHERIT APIs, where grants on the target are "
        "inherited to all projects in the subtree, "
        "if applicable.",
        scope_types=["system", "domain"],
        operations=[
            {
    ...
    base.APIRule(
        ...
        "to the OS-INHERIT APIs, where grants on the target are "
        "inherited to all projects in the subtree, "
        "if applicable. In that case, revoking the role grant in "
    ...
    base.APIRule(
        ...
        "When a relationship exists between a prior role and "
        "an implied role and the prior role is assigned to a "
        "user, the user also assumes the implied role.",
        scope_types=["system"],
        operations=[
    ...
    base.APIRule(
        ...
        check_str=("role:reader and system_scope:all"),
        description="List associations between two roles. When a "
        "relationship exists between a prior role and an implied "
        "role and the prior role is assigned to a user, the user "
        "also assumes the implied role. This will return all the "
        "implied roles that would be assumed by the user who "
        "gets the specified prior role.",
        scope_types=["system"],
        operations=[
    ...
    base.APIRule(
        ...
        check_str=(
            ...
            "target.limit.project.domain_id)s) or (project_id:%("
            "target.limit.project_id)s and not None:%("
            "target.limit.project_id)s) "
        ),
    ...
    base.APIRule(
        ...
        check_str=(
            ...
            "domain_id:%(target.project.domain_id)s) or project_id:%("
            "target.project.id)s "
        ),
    ...
    base.APIRule(
        ...
        check_str=(
            "role:reader and system_scope:all or user_id:%("
            "target.trust.trustor_user_id)s or user_id:%("
            "target.trust.trustee_user_id)s "
        ),
    ...
    base.APIRule(
        ...
        check_str=(
            "(role:reader and system_scope:all) or (role:reader and "
            "token.domain.id:%(target.user.domain_id)s) or user_id:%("
            "target.user.id)s "
        ),
